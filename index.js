...
function replaceText(word, replacement, text) {
	...
	const replaceText = text.replace(word, replacement);
	return replaceText;
}
...
function checkWord(word, text) {
	...
	const checkTest = regex.test(word);
	return checkTest;
}
...
function countEmails(str) {
	...
	const countEmail = matches ? matches.length : 0;
	return countEmail;
}
...
function replaceWordOccurrences(str, word, newWord) {
	...
	const regex = new RegExp("word", "g");
	let replaceText = str.replaceAll(word, newWord);
	return replaceText;
}
